...
public:
    ...
    /*初始化*/
    void init(permm::Model* model, DAT* dat,int **,char**,
...
void TaggingDecoder::init(//每一个模型保存了三个文件，一个模型文件，一个双数组TRIE树文件，一个标签文件，最后一个是什么？？
        permm::Model* model,
        DAT* dat,
        int ** pocs_to_tags,
        char** label_info,
...
void TaggingDecoder::set_label_trans(){// label_trans是什么？
